...
{
    PRINTFUNC();
    if(msg->pin_req.info.cod == 0)
    {
        //jhlee. 2011.01.07 check cod
        error("pin_req Device class is %x - skip it.", msg->pin_req.info.cod);
    }
    else 
    {
...
void dtun_dm_sig_authorize_req(tDTUN_DEVICE_SIGNAL *msg)
{
	...
	{
	 //jhlee. 2011.01.07 check cod
	  error("authorize_req Device class is %x - skip it.", msg->authorize_req.info.cod);
	}
	else 
	{
...
void dtun_dm_sig_auth_comp(tDTUN_DEVICE_SIGNAL *msg)
{
    ...
    if( !st )
    {
        ...
        dummy_handle++;
    } else if (st == HCI_OE_USER_ENDED_CONNECTION || st == HCI_LMP_RESPONSE_TIMEOUT) {
...
static void dtun_dm_sig_ssp_cfm_req(tDTUN_DEVICE_SIGNAL *msg)
{
	...
	{
	 //jhlee. 2011.01.07 check cod
	  error("ssp_cfm_req Device class is %x - skip it.", msg->ssp_cfm_req.info.cod);
	}
	else 
	{

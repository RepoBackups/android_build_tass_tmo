...
//[[SISO BIDI Solution Changes for Callisto <Srini.siddi> Start
#include "localizationGlyph.c"
#include <cutils/log.h>
//]]SISO BIDI Solution Changes for Callisto <Srini.siddi> End
#endif
using namespace std;
using namespace WTF;
...
#if defined(FEATURE_MEA_JP)
//[[SISO BIDI Solution Changes for Callisto <Srini.siddi> Start
	int nSrcLen = m_text->length();
	const UChar * srcSTR = m_text->characters();
		
	UChar * tmpBuff = (UChar *) malloc ( (nSrcLen+1) <<1);
	memcpy(tmpBuff,srcSTR,nSrcLen<<1);
	tmpBuff[nSrcLen] = '\0';
	int newlen = 0;
if (hasArabicLetters ((unsigned short *)tmpBuff, nSrcLen))
	...
	m_text = StringImpl::create (tmpBuff, nSrcLen);
	free(tmpBuff);
	//]]SISO BIDI Solution Changes for Callisto <Srini.siddi> End
#endif
